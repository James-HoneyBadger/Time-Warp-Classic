...
class IntegrationTestRunner:
    ...
    def setup_test_environment(self):
        ...
        # Try to create root window for UI tests
        if TKINTER_AVAILABLE:
            try:
                ...
                print(f"⚠️ UI not available despite tkinter import: {e}")
                self.root = None
                self.ui_available = False
        else:
            self.root = None
    ...
    def test_ui_components(self):
        ...
        if not self.ui_available:
            print("⚠️ UI not available, skipping UI components test")
            self.results["skipped"] += 1
            return True
        ...
        try:
            ...
            editor = EnhancedCodeEditor(self.root)

            # Test basic functionality
            editor.insert_text("T: Hello\nE:")
            content = editor.get_text()

            if "T: Hello" not in content:
                return False
    ...
    def test_plugin_lifecycle(self):
        ...
        try:
            ...
            # Test initialization (pass None if UI not available)
            parent = self.root if self.ui_available else None
            result = plugin.initialize(parent)
            if not result:
                return False

            # Test activation (skip UI-dependent parts if no UI)
            if self.ui_available:
                result = plugin.activate()
                if not result:
                    return False

            # Test deactivation (skip if no UI)
            if self.ui_available:
